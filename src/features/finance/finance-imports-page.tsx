...
import { archiveStatementImportAction } from "@/features/finance/actions";
      ...
      <div className="rounded-[2rem] border border-[var(--border-soft)] bg-white px-6 py-6 shadow-[0_24px_50px_-35px_rgba(15,23,42,0.35)]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm leading-6 text-slate-600">Bring PDFs in here first, then let review and reporting happen elsewhere.</p>
          </div>
          <ActionPanel
            buttonLabel="Import PDF"
            title="Add a statement PDF"
            description="Queue another statement without keeping the form open all the time."
            variant="primary"
          ...
          <article key={item.id} className="rounded-[2rem] border border-[var(--border-soft)] bg-white px-6 py-6 shadow-[0_24px_50px_-35px_rgba(15,23,42,0.35)]">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">{item.source_file_name} · {item.statement_month.slice(0, 7)}</p>
